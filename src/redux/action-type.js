export const NAVBARTEXT  = 'NAVBARTEXT';//顶部导航文字显示部分
export const SHOWBACK = 'SHOWBACK';//导航栏的返回按钮是否显示
export const ERRMSG = 'ERRMSG';//返回错误信息
export const NAVLIST = 'NAVLIST' //navlist
// 登录状态
export const CLEARREDUX = 'CLEARREDUX';//退出登录清空redux
export const SUCCESS_LOGIN = 'SUCCESS_LOGIN';
export const CLEAR_LOGIN = 'CLEAR_LOGIN'
export const JOBLIST = 'JOBLIST'//获取新的职位
export const IFGETNEWJOB = 'IFGETNEWJOB'//判断是否获取新的职位
export const IFJOBEND = 'IFJOBEND'//判断职位是否获取完
export const SAVEPERSONSET = 'SAVEPERSONSET'//保存个人设置
export const SAVEBOSSADDJOB = 'SAVEBOSSADDJOB'//保存boss创建的职位
export const SAVEONLINECV = 'SAVEONLINECV'//保存在线简历
